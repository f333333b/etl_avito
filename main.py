...
from data.reference_data import dealerships, autoload_allowed_values
...
logging.basicConfig(
    ...
    level=logging.INFO,
    format=(
        '%(asctime)s | %(levelname)-8s| %(name)-15s| %(lineno)-4d| '
...
def load_pipeline_config(config_path: str = 'pipeline_config.yaml') -> dict:
    ...
    try:
        ...
        logger.error(
            f"Конфигурационный YAML-файл не найден: {config_path}. Ошибка {e}"
        )
        raise
